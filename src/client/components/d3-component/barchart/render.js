...
render.animate = (id, data, options) => {
  ...
  update.transition()
    .attr('transform', (d, i) => `translate(0, ${(i + 1) * height * 0.07})`)
  update.select('rect').transition() // update
    .duration(duration)
    .attr('width', (d, i) => linear(d.number))
  ...
  const g = enter.append('g') // enter
    .attr('transform', (d, i) => `translate(0, ${(i + 1) * height * 0.07})`) // 矩形之间的间距
  g.append('rect')
    .attr('width', 0) // 矩形宽
    .attr('height', height * 0.01) // 矩形高
    .attr('fill', '#06D6A0')
    .transition()
    .duration(duration)
    .attr('width', (d, i) => linear(d.number));
